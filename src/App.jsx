...
// ===== قوالب جاهزة =====
const TPL_CATS = [
  { id:"portrait", icon:"👤", label:{ ar:"بورتريه وأشخاص", en:"Portraits", ku:"پۆرتریت", fa:"چهره‌ها", ur:"پورٹریٹ", tr:"Portreler", fr:"Portraits", es:"Retratos", de:"Porträts", ru:"Портреты", hi:"पोर्ट्रेट" } },
  { id:"business", icon:"🏢", label:{ ar:"تجاري", en:"Business", ku:"بازرگانی", fa:"تجاری", ur:"کاروبار", tr:"İş", fr:"Business", es:"Negocios", de:"Business", ru:"Бизнес", hi:"व्यापार" } },
  { id:"art", icon:"🎨", label:{ ar:"فني وإبداعي", en:"Art", ku:"هونەری", fa:"هنری", ur:"فن", tr:"Sanat", fr:"Art", es:"Arte", de:"Kunst", ru:"Арт", hi:"कला" } },
  { id:"nature", icon:"🌄", label:{ ar:"مناظر وطبيعة", en:"Nature", ku:"سروشت", fa:"طبیعت", ur:"مناظر", tr:"Doğa", fr:"Nature", es:"Naturaleza", de:"Natur", ru:"Природа", hi:"प्रकृति" } },
];
const TEMPLATES = [
  // بورتريه
        ...
        <section id="studio" className="studio">
          <div className="mode-row">
            ...
            <button className={"mode " + (mode === "video" ? "on" : "") + (isFree ? " locked-mode" : "")}
              ...
              <IconVideo /> {t.video}
              {isFree && <span className="lock-badge">🔒 {t.locked}</span>}
            </button>
          </div>
                ...
                {creations.filter((c) => !favOnly || c.favorite).map((c) => (
                  <div key={c.id} className="mw-card">
                    <div className="mw-media"
                      ...
                      {c.favorite && <span className="mw-fav-flag" aria-hidden="true">★</span>}
            ...
            <div className="tpl-cats">
              {TPL_CATS.map((c) => (
                <button key={c.id} className={"tpl-cat " + (tplCat === c.id ? "on" : "")} onClick={() => setTplCat(c.id)}>
                  <span className="tpl-cat-ic">{c.icon}</span>{c.label[lang] || c.label.ar}
                </button>
              ))}
            </div>
      ...
      <div className="ed-modal" onClick={(e) => e.stopPropagation()}>
        <div className="ed-topbar">
          ...
          <div className="ed-top-actions">
            <button className="ed-btn" onClick={resetAll}>↺ إعادة تعيين</button>
            <button className="ed-btn ed-btn-gold" onClick={download} disabled={busy || !loaded}>{busy ? "…" : "⬇ تنزيل"}</button>
              ...
              {tab === "crop" && (
                <>
                  ...
                  <div className="ed-btn-row">
                    <button className="ed-tool" onClick={() => setRotation((r) => (r + 270) % 360)}>↶ يسار</button>
                    <button className="ed-tool" onClick={() => setRotation((r) => (r + 90) % 360)}>↷ يمين</button>
                  </div>
                  <div className="ed-btn-row">
                    <button className={"ed-tool" + (flipH ? " on" : "")} onClick={() => setFlipH((f) => !f)}>⇆ قلب أفقي</button>
                    <button className={"ed-tool" + (flipV ? " on" : "")} onClick={() => setFlipV((f) => !f)}>⇅ قلب عمودي</button>
                  </div>
...
.tayf-root{
  --ink:#0A0A0C; --ink2:#131318; --surface:#16161C; --surface2:#1D1D25;
  --line:rgba(255,255,255,.11); --text:#F4F3F1; --muted:#A8A69C; --gold:#C9A86A; --spectrum:${SPECTRUM};
...
.spectrum-canvas{position:fixed;inset:0;z-index:0;pointer-events:none;width:100%;height:100%;
  background:radial-gradient(120% 90% at 50% 20%,#15131D 0%,#0C0B11 55%,#070608 100%)}
...
.sub{color:#CFCEC6;font-size:clamp(16.5px,2vw,20px);max-width:600px;margin:24px auto 0;line-height:1.9;font-weight:500}
...
.enhance{display:inline-flex;align-items:center;gap:8px;background:rgba(157,78,255,.14);border:1px solid rgba(157,78,255,.4);color:#D9C7FF;font-family:inherit;font-size:14.5px;font-weight:600;padding:9px 16px;border-radius:999px;cursor:pointer;transition:.2s}
...
.enhanced-head{display:flex;align-items:center;justify-content:space-between;font-size:13px;color:#C9A8FF;margin-bottom:8px}
...
.dur-btn.active{border-color:transparent;background:var(--spectrum);color:#0A0A0C;box-shadow:0 4px 16px rgba(157,78,255,.25)}
...
.generate{margin-top:26px;width:100%;display:flex;align-items:center;justify-content:center;gap:10px;background:var(--spectrum);border:none;color:#0B0B10;font-family:'Cairo','Noto Sans',sans-serif;font-weight:800;font-size:18px;padding:16px;border-radius:14px;cursor:pointer;transition:.2s;box-shadow:0 14px 40px -10px rgba(157,78,255,.6)}
.generate:hover{transform:translateY(-1px);box-shadow:0 18px 50px -10px rgba(157,78,255,.75)}
...
.mw-copy:hover{border-color:rgba(255,255,255,.3)}
...
.feat-ic{display:inline-flex;width:46px;height:46px;align-items:center;justify-content:center;border-radius:13px;background:rgba(157,78,255,.14);color:#C9A8FF;margin-bottom:16px}
...
.footer-copy{color:#6E6C7E;font-size:12.5px;text-align:center}
...
.acct-credits{display:inline-flex;align-items:center;gap:6px;font-size:13px;color:#D9C7FF;
...
.trial-cta{margin-top:26px;display:inline-flex;align-items:center;gap:9px;background:var(--spectrum);
  border:none;color:#0B0B10;font-family:'Cairo','Noto Sans',sans-serif;font-weight:800;font-size:16.5px;
  padding:14px 28px;border-radius:999px;cursor:pointer;transition:.2s;box-shadow:0 14px 40px -10px rgba(157,78,255,.6)}
.trial-cta:hover{transform:translateY(-1px);box-shadow:0 18px 50px -10px rgba(157,78,255,.75)}
...
.cost-hint svg{width:14px;height:14px;color:#C9A8FF}
...
.modal-close:hover{color:var(--text);background:var(--surface2);border-color:rgba(255,255,255,.28);transform:rotate(90deg)}
...
.modal-submit{margin-top:16px;width:100%;background:var(--spectrum);border:none;color:#0B0B10;
  ...
  cursor:pointer;transition:.2s;box-shadow:0 12px 34px -12px rgba(157,78,255,.6)}
...
.plan-credits{font-size:14px;color:#C9A8FF}
...
.admin-search-clear:hover{color:var(--text);border-color:rgba(255,255,255,.28)}
...
.admin-badge-ok{background:rgba(52,211,153,.14);color:#5EEAD4;border:1px solid rgba(52,211,153,.3)}
.admin-badge-mut{background:var(--surface2);color:var(--muted);border:1px solid var(--line)}
.admin-badge-sub{background:rgba(157,78,255,.16);color:#C9A8FF;border:1px solid rgba(157,78,255,.4)}
.admin-user-meta{display:flex;align-items:center;gap:8px;flex-wrap:wrap;font-size:12.5px;color:var(--muted)}
.admin-credits{display:inline-flex;align-items:center;gap:5px;color:#D9C7FF}
...
.admin-btn-danger{color:#FF8A8A;border-color:rgba(255,90,90,.3)}
...
/* ===== محرّر الصور ===== */
.ed-overlay{position:fixed;inset:0;z-index:200;background:rgba(6,6,9,.86);backdrop-filter:blur(8px);
  display:flex;align-items:center;justify-content:center;padding:18px}
.ed-modal{background:var(--ink);border:1px solid var(--line);border-radius:20px;width:min(1100px,100%);
  height:min(92vh,860px);display:flex;flex-direction:column;overflow:hidden;box-shadow:0 40px 120px -30px rgba(0,0,0,.9)}
...
.ed-btn:hover{border-color:rgba(255,255,255,.28);background:var(--surface2)}
...
.ed-canvas-host{position:relative;max-width:100%;max-height:100%;line-height:0;border-radius:12px;overflow:hidden;
  box-shadow:0 20px 60px -25px rgba(0,0,0,.9)}
...
.ed-tip{position:absolute;bottom:14px;left:50%;transform:translateX(-50%);font-size:11.5px;color:var(--muted);
  background:rgba(19,19,24,.85);padding:5px 12px;border-radius:999px;border:1px solid var(--line);white-space:nowrap}
...
.share-opt:hover{transform:translateY(-2px);border-color:rgba(255,255,255,.28);background:var(--surface2)}
...
.tpl-cat:hover{color:var(--text);border-color:rgba(255,255,255,.25)}
...
.ref-card{background:
    radial-gradient(120% 100% at 50% 0%, rgba(157,78,255,.12) 0%, rgba(19,19,24,0) 60%),
    var(--ink2);
...
.ref-copy-btn{display:inline-flex;align-items:center;gap:8px;background:var(--spectrum);border:none;color:#0B0B10;
